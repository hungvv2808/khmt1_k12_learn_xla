...
license_plate = cv2.imread("img/bienso.png", 0)
display(license_plate)

# reads in the car image
car = cv2.imread("img/bienso.jpg", 0)
display(car)
...
def FAST_n_BRIEF(img1, img2, n=70, threshold=85):
    ...
    result = cv2.drawMatches(mark1, kp1, mark2, kp2, matches[:min(n, len(matches))], None, matchColor=[0, 255, 0],
                             flags=2)
